	...
	return this.each(function () {
		...
		if($items.length != 0){
			...
			if (($items.length % visible) != 0) { // 마지막이 1개나 2개일 경우는 visible값도 1 또는 2
				...
				$slider.append(repeat('<li value="9999" class="scroll_empty" />', visible - ($items.length % visible))); // empty class추가 3 - (11 % 3)
...
function layer_popup(el){
	...
	$('.scroll_pop_layerSave').off().click(function(){
		...
		$('#scroll_btn_ul li.scroll_empty cloned').remove();
		$('#scroll_btn_ul li.scroll_empty').remove();
	...
	if(!($('#panel').hasClass('preview'))){
		...
		$(document).keydown(function(event) {
			 if (event.keyCode == '114' || event.keyCode == '82') {
				 if($('#panel-draggable-btn').hasClass('open') && $('.scroll_pop_layer').css('display') == 'none'){
		...
		////////////////////////////////////////////////////////////////
	}else { //preview에서 돌아갈 부분		
		// $('.scroll_infiniteCarousel').css('margin-left', '0px');
		
		for(var i=0;i<4;i++){
			// save at sessionStorage (key = itemID, value = item_url)
			sessionStorage.setItem("scroll_item_url"+i, 'preview'+i);
			// save at sessionStorage (keyword = user input value)
			sessionStorage.setItem("scroll_item_keyword"+i, 'preview'+i);
		}
		...
		$('.scroll_btn_ul li').unbind('click');
		var test = $('.scroll_wrapper');
		var $items1 = test.find('> ul');
		var $items = $items1.find('> li');
		
		$items.css('font-size', '16px');
		// $('.scroll_forward').off().trigger('click');
	}
});
