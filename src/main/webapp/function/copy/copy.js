var copy_init = (function(){
	...
	return {
		init : function(){
			// tooltip 초기화
			$Palette('.scroll_copy_button').tooltip();
			
			$('#scroll_text_url').val(location.href);
			// 복사 버튼 클릭시
			$Palette('.scroll_copy_button').off().click(function(){
...
$(document).ready(function(){
	copy_init.init();
	 //도움말 관련
	$Palette('.far.scroll_question_button').off().click(function(){
		if($('.scroll_question_popup').css('display') == 'none' ){
			$('.scroll_question_popup').fadeIn(500);
		}
	});
	
	$Palette('.scroll_question_popup_close').off().click(function(){
